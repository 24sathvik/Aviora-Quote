...
import { QuotationPdfDocument } from '../src/lib/pdf/QuotationPdfDocument'
import { InvoicePdfDocument } from '../src/lib/pdf/InvoicePdfDocument'
import { PaymentReceiptPdfDocument } from '../src/lib/pdf/PaymentReceiptPdfDocument'
import { PayslipPdfDocument } from '../src/lib/pdf/PayslipPdfDocument'
...
async function verifyPdfs() {
  ...
  const quoteBuffer = await renderToBuffer(
    React.createElement(QuotationPdfDocument, { quotation: sampleQuotation, settings }) as any
  )
  ...
  const invoiceBuffer = await renderToBuffer(
    React.createElement(InvoicePdfDocument, { invoice: sampleInvoice, settings }) as any
  )
  ...
  const receiptBuffer = await renderToBuffer(
    React.createElement(PaymentReceiptPdfDocument, { payment: samplePayment, settings }) as any
  )
  ...
  const payslipBuffer = await renderToBuffer(
    React.createElement(PayslipPdfDocument, { payslip: samplePayslip, settings }) as any
  )
