-- ==============================================================================
-- Migration 0013: Update public.create_invoice DDL for Manual Prefix Normalization
-- ==============================================================================
...
RETURNS TABLE(invoice_id uuid, invoice_no text, subtotal numeric, previous_outstanding numeric, gst_amount numeric, grand_total numeric, status text)
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  IF p_manual_invoice_no IS NOT NULL AND TRIM(p_manual_invoice_no) <> '' THEN
    ...
    -- Normalize: If raw digits are passed (e.g. '321'), prepend mandatory prefix AV/INV/{fy}/ and 5-digit padding
    ...
    -- Extract trailing integer and resync sequence with GREATEST
    IF v_invoice_no ~ '[0-9]+' THEN
